...
class SmartNotificationService {
  ...
  Future<void> checkSmartNotifications({
    ...
    try {
      await _evaluateAndSendSingle(goals: goals, healthProfile: healthProfile);
    } finally {
  ...
  Future<void> _evaluateAndSendSingle({
    ...
    final withinAllowedHours =
        now.hour >= _allowedStartHour && now.hour < _allowedEndHour;
    ...
    if (lastAnyStr != null) {
      ...
      if (lastAnyNotif != null &&
          now.difference(lastAnyNotif).inMinutes < _globalCooldownMinutes) {
        canSendAny = false;
      }
    }
    ...
    final bool dailyRemindersEnabled =
        prefs.getBool('daily_reminders') ?? false;
    final bool waterRemindersEnabled =
        prefs.getBool('water_reminders') ?? false;
    final bool calorieRemindersEnabled =
        prefs.getBool('calorie_reminders') ?? false;
    final bool proteinRemindersEnabled =
        prefs.getBool('protein_reminders') ?? false;
    final bool movementRemindersEnabled =
        prefs.getBool('movement_reminders') ?? false;
    final bool weightRemindersEnabled =
        prefs.getBool('weight_reminders') ?? false;

    // ── 4. Değerleri Al ──
    final double currentWater = goals?.waterConsumed ?? 0.0;
    final double waterGoal = (goals?.waterTarget ?? 0.0) > 0
        ? goals!.waterTarget.toDouble()
        : 2500.0;
    final double currentCalories = goals?.caloriesConsumed ?? 0.0;
    final double calorieGoal = (goals?.caloriesTarget ?? 0.0) > 0
        ? goals!.caloriesTarget.toDouble()
        : 2000.0;
    final double currentProtein = goals?.proteinConsumed ?? 0.0;
    final double proteinGoal = (goals?.proteinTarget ?? 0.0) > 0
        ? goals!.proteinTarget.toDouble()
        : 100.0;
    final double currentActivityMinutes = goals?.activityCompleted ?? 0.0;
    final double activityGoalMinutes = (goals?.activityTarget ?? 0.0) > 0
        ? goals!.activityTarget.toDouble()
        : 60.0;
    final DateTime? lastWeightUpdateDate =
    ...
    final calorieHighEligible =
        dailyRemindersEnabled &&
        calorieRemindersEnabled &&
        goals != null &&
    ...
    if (calorieHighEligible) {
      candidates.add(
        const _NotifCandidate(
          ...
          body:
              'Bugünkü kalori hedefini aştın. Kalan öğünlerinde daha hafif seçimler yapabilirsin.',
    ...
    final proteinLowEligible =
        dailyRemindersEnabled &&
        proteinRemindersEnabled &&
        goals != null &&
    ...
    if (proteinLowEligible) {
      candidates.add(
        const _NotifCandidate(
          ...
          body:
              'Bugünkü protein hedefinin gerisindesin. Protein ağırlıklı hafif bir tarif seçebilirsin.',
    ...
    if (proteinHighEligible) {
      candidates.add(
        const _NotifCandidate(
          ...
          body:
              'Bugünkü protein hedefini tamamladın. Dengeli ilerlemeye devam et.',
    ...
    if (calorieLowEligible) {
      candidates.add(
        const _NotifCandidate(
          ...
          body:
              'Bugünkü kalorin düşük görünüyor. Dengeli bir öğün ekleyebilirsin.',
    ...
    if (activityEligible) {
      candidates.add(
        const _NotifCandidate(
          ...
          body:
              'Bugün biraz hareketsiz kaldın. Kısa bir yürüyüş iyi gelebilir.',
    ...
    final bool weightOverdue =
        ...
        now.difference(lastWeightUpdateDate).inDays >= 7;
    final weightEligible =
        dailyRemindersEnabled &&
...
- userLoggedIn: ${Supabase.instance.client.auth.currentUser != null}
- waterValue: $currentWater
- calorieValue: $currentCalories
- proteinValue: $currentProtein
- activityValue: $currentActivityMinutes
- withinAllowedHours: $withinAllowedHours
- lastAnyNotificationAt: ${lastAnyNotif?.toIso8601String() ?? 'never'}
- candidateCount: ${candidates.length}
- skippedBecauseGlobalCooldown: ${!canSendAny}
- selectedNotificationType: ${candidates.isNotEmpty ? candidates.first.type.name : 'none'}
''');

    // ── 7. Gönderim Kararı ──
    if (!withinAllowedHours) {
      debugPrint(
        'Smart notification SKIPPED: outside allowed hours. reasonSkipped: outside_hours',
      );
      return;
    }
    ...
    if (!canSendAny) {
      debugPrint(
        'Smart notification SKIPPED: global cooldown active ($_globalCooldownMinutes min). reasonSkipped: global_cooldown',
      );
      return;
    }
    ...
    if (candidates.isEmpty) {
      debugPrint(
        'Smart notification SKIPPED: no eligible candidates. reasonSkipped: no_candidates',
      );
      return;
    }
