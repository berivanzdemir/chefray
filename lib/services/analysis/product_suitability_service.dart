...
class ProductSuitabilityService {
  ...
  String analyzeSuitability(
    ...
    if (product.sugars != null && product.sugars! > 15.0) {
      insights.add(
        "İlave şeker oranı yüksek görünüyor, porsiyon kontrolüne dikkat etmelisin.",
      );
    ...
    if (product.saturatedFat != null && product.saturatedFat! > 5.0) {
      insights.add(
        "Doymuş yağ içeriği fazla; sürekli tüketiminden kaçınmalısın.",
      );
    }
    ...
    if (product.salt != null && product.salt! > 1.5) {
      insights.add(
        "Tuz oranı yüksek, gün içindeki diğer öğünlerini dengelemelisin.",
      );
    }
    ...
    if (profile != null) {
      final userAllergies = profile.allergies
          .map((e) => e.toLowerCase())
          .toList();
      final productAllergies = product.allergens
          .map((e) => e.toLowerCase())
          .toList();
        ...
        if (productAllergies.contains(allergy) ||
            (allergy.contains('gluten') &&
                productAllergies.any(
                  (a) => a.contains('gluten') || a.contains('wheat'),
                ))) {
          insights.insert(
            0,
            "⚠️ DİKKAT: Profilindeki alerjen ($allergy) bu üründe bulunuyor!",
          );
        }
      }

      final preferences = profile.dietPreferences
          .map((e) => e.toLowerCase())
          .toList();
      if (preferences.any(
        (p) => p.contains('düşük karbonhidrat') || p.contains('low carb'),
      )) {
        if (product.carbs != null && product.carbs! > 20.0) {
          insights.add(
            "Düşük karbonhidrat hedefin için bu ürünün karbonhidrat değeri biraz yüksek.",
          );
        }
      }
    }
