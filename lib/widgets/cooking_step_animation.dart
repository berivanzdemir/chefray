...
class _CookingStepAnimationState extends State<CookingStepAnimation>
  ...
  CookingStepVisualType _getStepVisualType(String text, bool isLast) {
    ...
    t = t
        .replaceAll('süzme yoğurt', 'malzeme_yogurt')
    ...
    t = t
        .replaceAll('suyunu çek', 'suyunu_cekme_islemi')
        .replaceAll('suyunu cek', 'suyunu_cekme_islemi');

    t = t
        .replaceAll('bıçak değmeden', 'bicaksiz_islem')
        .replaceAll('bicak degmeden', 'bicaksiz_islem');

    t = t
        .replaceAll('tavaya yağ', 'tavaya_yag_alma')
        .replaceAll('tavaya yag', 'tavaya_yag_alma');

    if (t.contains('yufka') && t.contains('katla')) {
      t += ' yufka_katlama_kesme_islemi ';
    }
    if (t.contains('katlanır ve ikiye kesilir') ||
        t.contains('katlanir ve ikiye kesilir')) {
      t += ' yufka_katlama_kesme_islemi ';
    }
    ...
    Map<CookingStepVisualType, int> scores = {
      CookingStepVisualType.serving: calcScore(
        r'\b(servis|afiyet|sunum|tabaga al|sicak servis|soguk servis)',
      ),
      CookingStepVisualType.blending: calcScore(
        r'\b(blender|rondo|robot|robottan|pure|puruzsuz|mikser|ezici)',
      ),
      CookingStepVisualType.kneading: calcScore(r'\b(yogur)'),
      CookingStepVisualType.grating: calcScore(r'\b(rende)'),
      CookingStepVisualType.shredding: calcScore(
        r'\b(didik|lif lif|kucuk parcalara)',
      ),
      CookingStepVisualType.tearing: calcScore(
        r'\b(elle parcala|elle kopar|bicaksiz_islem|yapraklarina ayir|iri parcalara)',
      ),
      CookingStepVisualType.foldingCutting: calcScore(
        r'\b(yufka_katlama_kesme_islemi|ikiye kes|rulo yapilip kes|katlayip kes)',
      ),
      CookingStepVisualType.filling: calcScore(
        r'\b(doldur|ic harc|iclerine paylastir|icini|dolmalik)',
      ),
      CookingStepVisualType.coating: calcScore(
        r'\b(una bula|yumurtaya batir|galeta|pane|kapla|bulayin|sosa)',
      ),
      CookingStepVisualType.roasting: calcScore(r'\b(kozle|izgara|mangal)'),
      CookingStepVisualType.ovenCooking: calcScore(
        r'\b(firin|tepsi|pisirme kagidi|yagli kagit|derece|onceden isitilmis)',
      ),
      CookingStepVisualType.panHeating: calcScore(
        r'\b(tavaya_yag_alma|tavaya yag koy|tavaya zeytinyagi|tavaya tereyagi|tavayi isit|yag kizdir|alti acilir|altini ac|tava isininca)',
      ),
      CookingStepVisualType.panCooking: calcScore(
        r'\b(tavada|tavaya alip|kavur|sotele|kizart|kizar|arkali onlu|muhurle|az yagda)',
      ),
      CookingStepVisualType.boiling: calcScore(
        r'\b(hasla|kaynat|kaynama|kaynar|fokurda)',
      ),
      CookingStepVisualType.potCooking: calcScore(
        r'\b(tencere|kisik ates|suyunu_cekme_islemi|kapagi kapali|pilav gibi|kivam alana|renk alana)',
      ),
      CookingStepVisualType.cutting: calcScore(
        r'\b(dogra|kes|dilim|kup kup|ince ince|julyen|minik minik)',
      ),
      CookingStepVisualType.washing: calcScore(r'\b(yika|yikama|durula)'),
      CookingStepVisualType.straining: calcScore(
        r'\b(suz|suzun|suzulur|suzdur|suzgec)',
      ),
      CookingStepVisualType.sauceMixing: getSauceMixingScore(),
      CookingStepVisualType.mixing: calcScore(
        r'\b(karistir|cirp|harmanla|homojen|guzelce karistir)',
      ),
      CookingStepVisualType.shaping: calcScore(
        r'\b(sekil|yuvarla|beze|rulo|sar\b|sarin\b|sarip\b|durum|top haline)',
      ),
      CookingStepVisualType.resting: calcScore(
        r'\b(dinlendir|beklet|buzdolabi|mayalan|sogu|oda sicakliginda)',
      ),
      CookingStepVisualType.adding: calcScore(
        r'\b(ekle|ilave|uzerine|icine al|icerisine ekle)',
      ),
      CookingStepVisualType.genericCooking: calcScore(
        r'\b(al\b|alin\b|alip\b|koy\b|koyun\b|koyup\b|hazir|pis)',
      ),
    };

    bool hasStrongCooking = hasWord([
      'kavur',
      'sotele',
      'kizart',
      'pisir',
    ...
    if (hasStrongCooking) {
      if ((scores[CookingStepVisualType.panCooking] ?? 0) > 0)
        scores[CookingStepVisualType.panCooking] =
            scores[CookingStepVisualType.panCooking]! + 10;
      if ((scores[CookingStepVisualType.potCooking] ?? 0) > 0)
        scores[CookingStepVisualType.potCooking] =
            scores[CookingStepVisualType.potCooking]! + 10;
      if ((scores[CookingStepVisualType.panHeating] ?? 0) > 0)
        scores[CookingStepVisualType.panHeating] =
            scores[CookingStepVisualType.panHeating]! + 10;
      if ((scores[CookingStepVisualType.boiling] ?? 0) > 0)
        scores[CookingStepVisualType.boiling] =
            scores[CookingStepVisualType.boiling]! + 10;
      if ((scores[CookingStepVisualType.ovenCooking] ?? 0) > 0)
        scores[CookingStepVisualType.ovenCooking] =
            scores[CookingStepVisualType.ovenCooking]! + 10;
    }
    ...
    debugPrint(
      '[ChefRayAnimation] matchedCategories: ${matchedCategories.join(', ')}',
    );
    debugPrint(
      '[ChefRayAnimation] hasMappedWidget: true',
    ); // We guarantee this via exhaustive switch
...
class _Modern2DIllustrationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    switch (type) {
      ...
      case CookingStepVisualType.straining:
        _drawWashing(canvas, center);
        break;
      case CookingStepVisualType.cutting:
        _drawCutting(canvas, center);
        break;
      case CookingStepVisualType.grating:
        _drawGrating(canvas, center);
        break;
      case CookingStepVisualType.shredding:
        _drawShredding(canvas, center);
        break;
      case CookingStepVisualType.tearing:
        _drawTearing(canvas, center);
        break;
      case CookingStepVisualType.foldingCutting:
        _drawFoldingCutting(canvas, center);
        break;
      case CookingStepVisualType.adding:
        _drawAdding(canvas, center);
        break;
      case CookingStepVisualType.mixing:
        _drawMixing(canvas, center);
        break;
      case CookingStepVisualType.sauceMixing:
        _drawSauceMixing(canvas, center);
        break;
      case CookingStepVisualType.kneading:
        _drawKneading(canvas, center);
        break;
      case CookingStepVisualType.shaping:
        _drawShaping(canvas, center);
        break;
      case CookingStepVisualType.filling:
        _drawFilling(canvas, center);
        break;
      case CookingStepVisualType.coating:
        _drawCoating(canvas, center);
        break;
      case CookingStepVisualType.resting:
        _drawResting(canvas, center);
        break;
      case CookingStepVisualType.panHeating:
        _drawPanHeating(canvas, center);
        break;
      case CookingStepVisualType.panCooking:
        _drawPanCooking(canvas, center);
        break;
      case CookingStepVisualType.potCooking:
      case CookingStepVisualType.boiling:
        _drawPotCooking(canvas, center);
        break;
      case CookingStepVisualType.roasting:
        _drawRoasting(canvas, center);
        break;
      case CookingStepVisualType.ovenCooking:
        _drawOvenCooking(canvas, center);
        break;
      case CookingStepVisualType.blending:
        _drawBlending(canvas, center);
        break;
      case CookingStepVisualType.serving:
        _drawServing(canvas, center);
        break;
      case CookingStepVisualType.genericCooking:
        _drawGeneric(canvas, center);
        break;
    }
  ...
  void _drawWashing(Canvas canvas, Offset center) {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 4; j++) {
        _drawCircle(
          canvas,
          Offset(-30.0 + j * 20, 15.0 + i * 12),
          3,
          _darken(colanderColor),
        );
      }
    }
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 55), width: 50, height: 10),
      4,
      _darken(colanderColor),
    );
    ...
    for (int i = 0; i < 5; i++) {
      ...
      canvas.drawPath(
        dropPath,
        Paint()
          ..color = const Color(0xFF60A5FA).withValues(alpha: op)
          ..style = PaintingStyle.fill,
      );
    }
  ...
  void _drawCutting(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 20), width: 180, height: 25),
      8,
      boardColor,
    );
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 15), width: 180, height: 25),
      8,
      boardTop,
    );
    ...
    for (int i = 0; i < 4; i++) {
      ...
      _drawRoundedRect(
        canvas,
        Rect.fromCenter(center: Offset(x, -5), width: 18, height: 18),
        4,
        vegColor,
      );
    }
    ...
    _drawRoundedRect(
      canvas,
      const Rect.fromLTWH(50, -5, 45, 20),
      4,
  ...
  void _drawGrating(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 40), width: 180, height: 20),
      8,
      boardColor,
    );
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, -60), width: 30, height: 15),
      5,
      const Color(0xFF374151),
    );
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(30, 0), width: 60, height: 20),
      10,
    ...
    for (int i = 0; i < 4; i++) {
      _drawRoundedRect(
        canvas,
        Rect.fromCenter(
          center: Offset(-40.0 + i * 10, 25.0 + (i % 2) * 5),
          width: 10,
          height: 4,
        ),
        2,
  ...
  void _drawShredding(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 0), width: 80, height: 50),
      15,
      meatColor,
    );
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, -30), width: 8, height: 50),
      4,
      const Color(0xFFD1D5DB),
    );
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 5), width: 30, height: 8),
      2,
  ...
  void _drawTearing(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(-20, -10), width: 30, height: 20),
      10,
  ...
  void _drawFoldingCutting(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 30), width: 180, height: 20),
      8,
      boardColor,
    );
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(-20, 10), width: 100, height: 20),
      5,
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(
        center: Offset(-20 + foldPhase * 10, 5),
        width: 90,
        height: 10,
      ),
      5,
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(25, 10), width: 30, height: 15),
      4,
      const Color(0xFF374151),
    );
    canvas.restore();
  ...
  void _drawSauceMixing(Canvas canvas, Offset center) {
    ...
    final sauceColor = const Color(
      0xFFFDE047,
    ); // Yellowish sauce (mayo/mustard)

    final bowl = Path()
      ..moveTo(-50, 0)
      ..lineTo(50, 0)
      ..quadraticBezierTo(50, 40, 0, 40)
      ..quadraticBezierTo(-50, 40, -50, 0)
      ..close();
    _drawFlatShape(canvas, bowl, bowlColor);
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, 0), width: 100, height: 20),
      Paint()
        ..color = innerColor
        ..style = PaintingStyle.fill,
    );
    ...
    final sauce = Path()
      ..moveTo(-40, 0)
      ..quadraticBezierTo(0, math.sin(wavePhase) * 10, 40, 0)
      ..quadraticBezierTo(0, 15, -40, 0)
      ..close();
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, -30), width: 6, height: 40),
      3,
      const Color(0xFF9CA3AF),
    );
  ...
  void _drawMixing(Canvas canvas, Offset center) {
    ...
    _drawFlatShape(canvas, bowl, bowlColor);
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, 0), width: 160, height: 30),
      Paint()
        ..color = innerColor
        ..style = PaintingStyle.fill,
    );
  ...
  void _drawKneading(Canvas canvas, Offset center) {
    ...
    canvas.drawOval(
      Rect.fromCenter(
        center: Offset(0, -10 + (pressPhase * 5)),
        width: doughW,
        height: doughH,
      ),
      Paint()
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(-20, -10), width: 20, height: 35),
      10,
      const Color(0xFFFDBA74),
    );
    _drawRoundedRect(
  ...
  void _drawShaping(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(
        center: Offset(20 + rx, -25 + hPhase * 5),
        width: 40,
        height: 20,
      ),
      10,
  ...
  void _drawFilling(Canvas canvas, Offset center) {
    ...
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, -10), width: 80, height: 20),
      Paint()
        ..color = _darken(vegColor)
        ..style = PaintingStyle.fill,
    );
    ...
    for (int i = 0; i < 4; i++) {
      ...
      _drawCircle(
        ...
        Offset(x, y),
        5,
        fillingColor.withValues(alpha: math.sin(pPhase * math.pi)),
      );
    }
  ...
  void _drawOvenCooking(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 0), width: 140, height: 120),
      12,
      ovenColor,
    );
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 15), width: 100, height: 60),
      8,
      windowColor,
    );
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 15), width: 90, height: 50),
      6,
      const Color(0xFFF59E0B).withValues(alpha: 0.3 + glow * 0.3),
    );
  ...
  void _drawRoasting(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 10), width: 160, height: 5),
      2,
      grillColor,
    );
    for (int i = -3; i <= 3; i++) {
      _drawRoundedRect(
        canvas,
        Rect.fromCenter(center: Offset(i * 20.0, 10), width: 5, height: 20),
        2,
        grillColor,
      );
    }
    ...
    final veg = Path()
      ..moveTo(-40, 0)
      ..quadraticBezierTo(0, -30, 40, 0)
      ..quadraticBezierTo(0, 20, -40, 0)
      ..close();
  ...
  void _drawCoating(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 20), width: 100, height: 40),
      20,
      const Color(0xFFFDE68A),
    );
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(20, -30), width: 10, height: 40),
      5,
      const Color(0xFFD1D5DB),
    );
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(20, -5), width: 20, height: 15),
      2,
      const Color(0xFFFDBA74),
    );
    canvas.restore();
  ...
  void _drawAdding(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 30), width: 120, height: 80),
      12,
      potColor,
    );
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, -10), width: 120, height: 20),
      Paint()
        ..color = _darken(potColor)
        ..style = PaintingStyle.fill,
    );
  ...
  void _drawPanHeating(Canvas canvas, Offset center) {
    ...
    // Empty pan surface
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, -10), width: 140, height: 15),
      Paint()
        ..color = _lighten(panColor)
        ..style = PaintingStyle.fill,
    );
  ...
  void _drawPotCooking(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(-75, 10), width: 20, height: 30),
      4,
      _darken(potColor),
    );
    _drawRoundedRect(
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 30), width: 140, height: 90),
      16,
      potColor,
    );
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, -15), width: 140, height: 20),
      Paint()
        ..color = _darken(potColor)
        ..style = PaintingStyle.fill,
    );
    ...
    final lid = Path()
      ..moveTo(-75, 0)
      ..lineTo(75, 0)
      ..quadraticBezierTo(0, -30, -75, 0)
      ..close();
    _drawFlatShape(canvas, lid, _lighten(potColor));
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, -15), width: 20, height: 10),
      3,
      _darken(potColor),
    );
    canvas.restore();
  ...
  void _drawBlending(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 45), width: 120, height: 70),
      12,
      potColor,
    );
    canvas.drawOval(
    ...
    canvas.drawOval(
      Rect.fromCenter(center: Offset(0, 10 + wave), width: 100, height: 15),
      Paint()
        ..color = accent
        ..style = PaintingStyle.fill,
    );
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, -20), width: 16, height: 70),
      4,
      const Color(0xFF9CA3AF),
    );
    final head = Path()
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, -70), width: 30, height: 50),
      10,
      primary,
    );
    ...
    for (int i = 0; i < 3; i++) {
      ...
      _drawCircle(
        ...
        Offset(px, py),
        4,
        accent.withValues(alpha: math.sin(pPhase * math.pi)),
      );
    }
  ...
  void _drawServing(Canvas canvas, Offset center) {
    ...
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, 15), width: 200, height: 50),
      Paint()
    ...
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, -10), width: 140, height: 20),
      Paint()
    ...
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, -10), width: 110, height: 15),
      Paint()
        ..color = accent
        ..style = PaintingStyle.fill,
    );
  ...
  void _drawGeneric(Canvas canvas, Offset center) {
    ...
    _drawRoundedRect(
      canvas,
      Rect.fromCenter(center: const Offset(0, 20), width: 80, height: 20),
      4,
      primary,
    );
